...
def _round_to_psych(price: float) -> float:
    ...
    for ending in _PSYCH_ENDINGS:
        ...
        if candidate <= price:
            best = candidate
    # إذا كانت القيمة الصحيحة + 0.99 أكثر جاذبية من price
    # (مثلاً: price=200.5 → 199.99 أفضل من 200.49)
    candidate_below = (base - 1) + 0.99 if base >= 1 else price
    if abs(candidate_below - price) / price < 0.02:  # أقل من 2% فرق
...
def _detect_alert(
    ...
    if diff_pct >= rise_thresh:
        # المنافس أرخص بكثير → تهديد
...
class MerchantBrain:
    ...
    def classify_products(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        تصنيف المنتجات لأقسام القائمة الجانبية:
          🔴 high_price   — سعرنا أعلى من المنافس (خفض فوري)
          🟢 low_price    — سعرنا أقل ← يمكن رفعه
        ...
        if "strategy" in df.columns:
            sections["high_price"] = df[df["strategy"] == "undercut"].copy()
